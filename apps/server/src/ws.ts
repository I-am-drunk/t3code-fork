...
import { getVibecodeAuthStatus, rotateVibecodeApiKey } from "./vibecode/VibecodeAuthService.ts";
import {
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.vibecodeAuthStatus]: (_input) =>
          observeRpcEffect(
            WS_METHODS.vibecodeAuthStatus,
            Effect.promise(() => getVibecodeAuthStatus()),
            {
              "rpc.aggregate": "vibecode",
            },
          ),
        [WS_METHODS.vibecodeRotateApiKey]: (input) =>
          observeRpcEffect(
            WS_METHODS.vibecodeRotateApiKey,
            Effect.promise(() =>
        ...
        [WS_METHODS.vibecodeRuntimeStatus]: (input) =>
          observeRpcEffect(
            WS_METHODS.vibecodeRuntimeStatus,
            Effect.sync(() => getVibecodeRuntimeStatus(input)),
            {
              "rpc.aggregate": "vibecode",
            },
          ),
        [WS_METHODS.vibecodePreviewStatus]: (input) =>
          observeRpcEffect(
            WS_METHODS.vibecodePreviewStatus,
            Effect.sync(() => getVibecodePreviewStatus(input)),
            {
              "rpc.aggregate": "vibecode",
            },
          ),
        [WS_METHODS.vibecodePreviewOpen]: (input) =>
          observeRpcEffect(
            WS_METHODS.vibecodePreviewOpen,
            Effect.sync(() => {
              const status = getVibecodePreviewStatus(input);
              return {
                opened: status.status === "ready" && status.url !== undefined,
                status,
                ...(status.status === "ready" && status.url !== undefined
                  ? {}
                  : { message: status.message ?? "Vibecode preview is not ready yet." }),
              };
            }),
            {
              "rpc.aggregate": "vibecode",
        ...
        [WS_METHODS.vibecodeSyncStatus]: (input) =>
          observeRpcEffect(
            WS_METHODS.vibecodeSyncStatus,
            Effect.sync(() => getVibecodeSyncStatus(input)),
            {
              "rpc.aggregate": "vibecode",
            },
          ),
