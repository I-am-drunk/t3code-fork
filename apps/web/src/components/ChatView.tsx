...
import { ChevronDownIcon, KeyRoundIcon, TriangleAlertIcon, WifiOffIcon } from "lucide-react";
...
export default function ChatView(props: ChatViewProps) {
  ...
  const isVibecodeSelected =
    activeProviderStatus?.driver === "vibecode" ||
    activeProviderInstanceId?.startsWith("vibecode") ||
    activeThread?.modelSelection.model.toLowerCase().includes("vibecode") === true;
  const [vibecodeAuthStatus, setVibecodeAuthStatus] = useState<VibecodeAuthStatusResult | null>(
    null,
  );
  const [vibecodeApiKeyDraft, setVibecodeApiKeyDraft] = useState("");
  const [vibecodeRotateBusy, setVibecodeRotateBusy] = useState(false);
  const [vibecodeRotateMessage, setVibecodeRotateMessage] = useState<string | null>(null);
  const [vibecodeRuntimeStatus, setVibecodeRuntimeStatus] = useState<VibecodeRuntimeStatus | null>(
    null,
  );
  const [vibecodePreviewStatus, setVibecodePreviewStatus] = useState<VibecodePreviewStatus | null>(
    null,
  );
  const [vibecodeSyncStatus, setVibecodeSyncStatus] = useState<VibecodeSyncStatus | null>(null);
  useEffect(() => {
    if (!isVibecodeSelected) {
      setVibecodeRuntimeStatus(null);
      setVibecodePreviewStatus(null);
      setVibecodeSyncStatus(null);
      return;
    }
    ...
    const refresh = async () => {
      ...
      if (!api) return;
      const runtimeInput = activeThread?.id ? { threadId: activeThread.id } : {};
      try {
        const [authStatus, runtimeStatus, previewStatus, syncStatus] = await Promise.all([
          api.server.vibecodeAuthStatus(),
          api.server.vibecodeRuntimeStatus(runtimeInput),
          api.server.vibecodePreviewStatus(runtimeInput),
          api.server.vibecodeSyncStatus(runtimeInput),
        ]);
        if (!cancelled) {
          setVibecodeAuthStatus(authStatus);
          setVibecodeRuntimeStatus(runtimeStatus);
          setVibecodePreviewStatus(previewStatus);
          setVibecodeSyncStatus(syncStatus);
        }
      } catch (error) {
        if (!cancelled) {
  ...
  }, [activeThread?.id, isVibecodeSelected]);
  const rotateVibecodeApiKey = useCallback(async () => {
    const apiKey = vibecodeApiKeyDraft.trim();
    if (!apiKey) {
      setVibecodeRotateMessage("Enter a Vibecode API key.");
      return;
    }
    const api = readLocalApi();
    if (!api) {
      setVibecodeRotateMessage("Local backend is not connected.");
      return;
    }
    setVibecodeRotateBusy(true);
    setVibecodeRotateMessage(null);
    try {
      const result = await api.server.vibecodeRotateApiKey({
        apiKey,
        ...(vibecodeRuntimeStatus?.projectId ? { projectId: vibecodeRuntimeStatus.projectId } : {}),
      });
      setVibecodeAuthStatus({
      ...
      if (result.accepted) setVibecodeApiKeyDraft("");
    } catch (error) {
      setVibecodeRotateMessage(
        error instanceof Error ? error.message : "Unable to rotate Vibecode API key.",
      );
    } finally {
      setVibecodeRotateBusy(false);
    }
  }, [vibecodeApiKeyDraft, vibecodeRuntimeStatus?.projectId]);
  const openVibecodePreview = useCallback(() => {
    const api = readLocalApi();
    if (!api) {
      setVibecodeRotateMessage("Local backend is not connected.");
      return;
    ...
    const runtimeInput = activeThread?.id ? { threadId: activeThread.id } : {};
    void api.server
      .vibecodePreviewOpen(runtimeInput)
      .then(async (result) => {
        setVibecodePreviewStatus(result.status);
        if (result.opened && result.status.url) {
          await api.shell.openExternal(result.status.url);
          return;
        }
        setVibecodeRotateMessage(
          result.message ?? result.status.message ?? "Vibecode preview is not ready yet.",
        );
      })
      .catch((error: unknown) => {
        setVibecodeRotateMessage(
          error instanceof Error ? error.message : "Unable to open Vibecode preview.",
        );
      });
  }, [activeThread?.id]);
  const vibecodeRecoveryBannerItems = useMemo<ComposerBannerStackItem[]>(() => {
    ...
    const isExhausted = vibecodeAuthStatus?.status === "exhausted";
    return [
      {
        id: `vibecode-key-recovery:${vibecodeAuthStatus?.status ?? "loading"}`,
        variant: isExhausted ? "warning" : "error",
        icon: <KeyRoundIcon />,
        title: isExhausted ? "Vibecode credits are exhausted" : "Vibecode API key required",
        description: (
          <div className="space-y-2">
            <p>
              {vibecodeAuthStatus?.message ??
                "Add a valid Vibecode API key before sending with a Vibecode model."}
            </p>
            <div className="flex flex-col gap-2 sm:flex-row">
              <Input
                nativeInput
                type="password"
                value={vibecodeApiKeyDraft}
                onChange={(event) => setVibecodeApiKeyDraft(event.currentTarget.value)}
                placeholder="vibecode_..."
                aria-label="New Vibecode API key"
                disabled={vibecodeRotateBusy}
              />
              <Button
                size="sm"
                onClick={() => void rotateVibecodeApiKey()}
                disabled={vibecodeRotateBusy}
              >
                {vibecodeRotateBusy ? "Checking..." : "Use key"}
              </Button>
            </div>
            {vibecodeRotateMessage ? (
              <p className="text-muted-foreground text-xs">{vibecodeRotateMessage}</p>
            ) : null}
          </div>
        ),
      },
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      ...
      >
        <ChatHeader
          ...
          gitCwd={gitCwd}
          diffOpen={diffOpen}
          vibecodeSelected={isVibecodeSelected}
          vibecodeAuthStatus={vibecodeAuthStatus}
          vibecodeRuntimeStatus={vibecodeRuntimeStatus}
          vibecodePreviewStatus={vibecodePreviewStatus}
          vibecodeSyncStatus={vibecodeSyncStatus}
          ...
          onOpenPreview={openVibecodePreview}
        />
      </header>
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          >
            <div className="relative isolate">
              <ComposerBannerStack className="relative z-0" items={vibecodeRecoveryBannerItems} />
