...
function VibecodeApiKeySection({
  instance,
  displayName,
}: {
  readonly instance: ProviderInstanceConfig;
  ...
  const [status, setStatus] = useState<VibecodeAuthStatusResult | null>(null);
  const [apiKey, setApiKey] = useState("");
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState<string | null>(null);
  const projectId = readConfigString(instance.config, "projectId");

  useEffect(() => {
    let cancelled = false;
    const api = readLocalApi();
    if (!api) {
      setMessage("Local backend is not connected.");
      return;
    }
    void api.server
      .vibecodeAuthStatus()
      ...
      .catch((error: unknown) => {
        if (!cancelled) {
          setMessage(
            error instanceof Error ? error.message : "Unable to read Vibecode auth status.",
          );
        }
      });
    return () => {
  ...
  const saveApiKey = async () => {
    ...
    if (!trimmed) {
      setMessage("Enter a Vibecode API key.");
      return;
    }
    const api = readLocalApi();
    if (!api) {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "Unable to save Vibecode API key.");
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="border-t border-border/60 px-4 py-3 sm:px-5">
      <div className="rounded-lg border border-border/70 bg-muted/20 p-3">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <p className="text-xs font-semibold text-foreground">Vibecode API key</p>
              <Badge variant={badgeVariant} size="sm">
                {statusLabel}
              </Badge>
            </div>
            <p className="text-xs text-muted-foreground">
              Stored in the local T3 Code key pool. This is separate from provider project settings.
            </p>
          </div>
        </div>
        ...
        {(message ?? status?.message) ? (
          <p className="mt-2 text-xs text-muted-foreground">{message ?? status?.message}</p>
        ) : null}
      </div>
    </div>
  );
      ...
      <Collapsible open={isExpanded} onOpenChange={onExpandedChange}>
        <CollapsibleContent>
          <div className="space-y-0">
            ...
            {instance.driver === "vibecode" ? (
              <VibecodeApiKeySection instance={instance} displayName={displayName} />
            ) : null}
