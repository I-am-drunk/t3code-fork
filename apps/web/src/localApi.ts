...
function createBrowserLocalApi(rpcClient?: WsRpcClient): LocalApi {
  return {
    ...
    server: {
      ...
      vibecodeAuthStatus: () =>
        rpcClient
          ? rpcClient.server.vibecodeAuthStatus()
          : Promise.reject(unavailableLocalBackendError()),
      vibecodeRotateApiKey: (input) =>
        rpcClient
          ? rpcClient.server.vibecodeRotateApiKey(input)
          : Promise.reject(unavailableLocalBackendError()),
      vibecodeRuntimeStatus: (input) =>
        rpcClient
          ? rpcClient.server.vibecodeRuntimeStatus(input)
          : Promise.reject(unavailableLocalBackendError()),
      vibecodePreviewStatus: (input) =>
        rpcClient
          ? rpcClient.server.vibecodePreviewStatus(input)
          : Promise.reject(unavailableLocalBackendError()),
      vibecodePreviewOpen: (input) =>
        rpcClient
          ? rpcClient.server.vibecodePreviewOpen(input)
          : Promise.reject(unavailableLocalBackendError()),
      vibecodeSyncStatus: (input) =>
        rpcClient
          ? rpcClient.server.vibecodeSyncStatus(input)
          : Promise.reject(unavailableLocalBackendError()),
    },
  };
}
